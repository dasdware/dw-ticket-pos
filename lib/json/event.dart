...
class JSONEvent {
  ...
  Event toEvent() {
    Event eventResult = Event(name, DateTime.tryParse(timestamp),
        this.tickets.map((ticket) => ticket.toTicket()).toList());
  ...
  JSONEvent.fromEvent(Event event) :
    ...
    tickets = event.availableTickets.map((ticket) => JSONTicket.fromTicket(ticket)).toList(),
    bookings = event.entries.map((booking) => JSONBooking.fromBooking(booking)).toList();
